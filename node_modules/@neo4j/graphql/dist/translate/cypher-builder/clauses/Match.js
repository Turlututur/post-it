"use strict";
/*
 * Copyright (c) "Neo4j"
 * Neo4j Sweden AB [http://neo4j.com]
 *
 * This file is part of Neo4j.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.OptionalMatch = exports.Match = void 0;
const Pattern_1 = require("../Pattern");
const Clause_1 = require("./Clause");
const utils_1 = require("../utils/utils");
const WithReturn_1 = require("./mixins/WithReturn");
const apply_mixin_1 = require("./utils/apply-mixin");
const WithWhere_1 = require("./mixins/WithWhere");
const WithSet_1 = require("./mixins/WithSet");
const WithWith_1 = require("./mixins/WithWith");
const Delete_1 = require("../sub-clauses/Delete");
const Remove_1 = require("../sub-clauses/Remove");
class Match extends Clause_1.Clause {
    constructor(variable, parameters = {}, parent) {
        super(parent);
        if (variable instanceof Pattern_1.Pattern) {
            this.pattern = variable;
        }
        else {
            this.pattern = new Pattern_1.Pattern(variable).withParams(parameters);
        }
    }
    delete(...deleteInput) {
        this.createDeleteClause(deleteInput);
        return this;
    }
    detachDelete(...deleteInput) {
        const deleteClause = this.createDeleteClause(deleteInput);
        deleteClause.detach();
        return this;
    }
    remove(...properties) {
        this.removeClause = new Remove_1.RemoveClause(this, properties);
        return this;
    }
    getCypher(env) {
        const nodeCypher = this.pattern.getCypher(env);
        const whereCypher = (0, utils_1.compileCypherIfExists)(this.whereSubClause, env, { prefix: "\n" });
        const returnCypher = (0, utils_1.compileCypherIfExists)(this.returnStatement, env, { prefix: "\n" });
        const setCypher = (0, utils_1.compileCypherIfExists)(this.setSubClause, env, { prefix: "\n" });
        const withCypher = (0, utils_1.compileCypherIfExists)(this.withStatement, env, { prefix: "\n" });
        const deleteCypher = (0, utils_1.compileCypherIfExists)(this.deleteClause, env, { prefix: "\n" });
        const removeCypher = (0, utils_1.compileCypherIfExists)(this.removeClause, env, { prefix: "\n" });
        return `MATCH ${nodeCypher}${whereCypher}${setCypher}${removeCypher}${deleteCypher}${withCypher}${returnCypher}`;
    }
    createDeleteClause(deleteInput) {
        this.deleteClause = new Delete_1.DeleteClause(this, deleteInput);
        return this.deleteClause;
    }
}
exports.Match = Match;
(0, apply_mixin_1.applyMixins)(Match, [WithReturn_1.WithReturn, WithWhere_1.WithWhere, WithSet_1.WithSet, WithWith_1.WithWith]);
class OptionalMatch extends Match {
    getCypher(env) {
        const matchStr = super.getCypher(env);
        return `OPTIONAL ${matchStr}`;
    }
}
exports.OptionalMatch = OptionalMatch;
//# sourceMappingURL=Match.js.map