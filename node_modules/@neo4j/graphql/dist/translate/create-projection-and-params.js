...
const create_projection_subquery_1 = require("./projection/elements/create-projection-subquery");
function createProjectionAndParams({ resolveTree, node, context, chainStr, varName, literalElements, resolveType, inRelationshipProjection, isRootConnectionField, }) {
    function reducer(res, field) {
        ...
        if (relationField) {
            ...
            const { inStr, outStr } = (0, get_relationship_direction_1.getRelationshipDirectionStr)(relationField, field.args);
            ...
            if (relationField.union) {
                ...
                const unionStrs = [
                    `${alias}: ${!isArray ? "head(" : ""} [${param} IN [(${chainStr || varName})${inStr}${relTypeStr}${outStr}(${param})`,
                ...
                const headStrs = referenceNodes.map((refNode) => {
                    ...
                    const refNodeInterfaceNames = refNode.interfaces.map((implementedInterface) => implementedInterface.name.value);
                ...
                if (optionsInput) {
                    const offsetLimit = (0, pagination_1.createOffsetLimitStr)({
...
function createNodeWhereAndParams({ whereInput, varName, context, node, authValidateStrs, chainStr, }) {
    ...
    let params = {};
    if (whereInput) {
        const whereAndParams = (0, create_where_and_params_1.default)({
            context,
            node,
            varName,
            whereInput,
